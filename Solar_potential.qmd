...
foobar <- tribble(~Index, ~Cost, ~Savings, ~InitialKwhPerYear)
for (i in 1:length(foo[[1]][["solarPotential"]][["financialAnalyses"]])) {
  ...
  print(paste(Index, Cost, Savings, InitialKwhPerYear))
  foobar <- bind_rows(foobar, tribble(~Index, ~Cost, ~Savings, ~InitialKwhPerYear, 
                                       Index, Cost, Savings, InitialKwhPerYear))
}
