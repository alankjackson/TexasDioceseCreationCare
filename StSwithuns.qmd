...
----
Likelihood                Severity
None, Low, medium, high   minor, serious, extensive, catastrophic
Flooding: Any=Major, >2 feet = Extensive, >5 feet = Catastrophic
Coastal flooding
Drought
Hurricanes - cat 1
Hurricanes - cat 2
...
df <- readxl::read_excel(paste0(out_path, "Risk_Matrix.xls")) %>% 
  ...
  mutate(WildFire_FirstSt_Like=case_when(
    WildFire_FirstSt_Like==0 ~ "None", 
    WildFire_FirstSt_Like<10 ~ "Low", 
    WildFire_FirstSt_Like>50 ~ "High",
    .default="Medium")) %>% 
  mutate(WildFire_FirstSt_Severe=if_else(WildFire_FirstSt_Like=="0%", "Minor", "Medium")) %>% 
  mutate(Flood_First_Like=case_when(
    Flood_First_Like==0 ~ "None", 
    Flood_First_Like<10 ~ "Low", 
    Flood_First_Like>50 ~ "High",
    .default="Medium")) %>% 
  mutate(Flood_First_Max_Like=case_when(
    Flood_First_Max_Like==0 ~ "None", 
    Flood_First_Max_Like<10 ~ "Low", 
    Flood_First_Max_Like>50 ~ "High",
    .default="Medium")) %>% 
  mutate(Flood_First_Max_Severe=case_when(
    Flood_First_Max_Severe==0 ~ "None", 
    Flood_First_Max_Severe<=2 ~ "Major", 
    Flood_First_Max_Severe>5 ~ "Extensive",
    .default="Catastrophic")) %>% 
  mutate(Wind_Like=case_when(
    Wind_Like==0 ~ "None", 
    Wind_Like<10 ~ "Low", 
    Wind_Like>50 ~ "High",
    .default="Medium")) %>% 
  mutate(Flood_First_Severe="Major")
...
df2 <- df %>% 
  ...
  mutate(Disaster=stringr::str_replace(Disaster, "Flood_First", "Flood 6 inches")) %>% 
  mutate(Disaster=stringr::str_replace(Disaster, "Flood_First_Max", "Flood Worst Case")) %>% 
  mutate(Disaster=stringr::str_replace(Disaster, "WildFire_FirstSt", "WildFire 1st")) %>% 
  mutate(Disaster=stringr::str_replace(Disaster, "Wind", "Tropical Wind")) %>% 
...
maketable <- function(df){
  ...
  df %>%
    ...
    tab_footnote(
    footnote = md("<10% over 30 years"),
    locations = cells_column_labels(columns = Low)
  ) %>% 
    tab_footnote(
    footnote = md("10%-50% over 30 years"),
    locations = cells_column_labels(columns = Medium)
  ) %>% 
    tab_footnote(
    footnote = md("\\>50% over 30 years"),
    locations = cells_column_labels(columns = High)
  ) %>% 
    tab_footnote(
    footnote = md("<$100,000"),
    locations = cells_column_labels(columns = Major)
  ) %>% 
    tab_footnote(
    footnote = md("\\>$100,000"),
    locations = cells_column_labels(columns = Extensive)
  ) %>% 
    tab_options(footnotes.multiline = FALSE) %>% 
    tab_source_note(source_note = paste("Best guess based on nultiple sources.",
                    "Note that Heat, Cold, and Power Outages are issues for all parishes."))
      # data_color(
      #   columns = High,
      #   rows = matches("X"),
      #   # method = "numeric",
...
Worsen <- tribble(~Worsen,
                  'Unk', 'Yes', "Yes", "Yes", "Yes", "Yes", "Yes", "No",
                  "No", "Yes", "Yes", "Unk", "Yes")
...
for (i in 1:163){
  ...
  mutate(Disaster=stringr::str_replace(Disaster, "WildFire_FirstSt", "WildFire 1st")) %>% 
  mutate(Disaster=stringr::str_replace(Disaster, "Wind", "Tropical Wind")) %>% 
  mutate(Disaster=stringr::str_replace(Disaster, "Hurricane_Surge", "Surge Cat ")) %>% 
  mutate(Severity=stringr::str_replace(Severity, "Minimal", "Minor"))  
  # head(5) %>% 
  # pivot_longer(-Parish, 
  #              names_to=c("Disaster", ".value"),
