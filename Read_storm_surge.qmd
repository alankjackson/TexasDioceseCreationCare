...
st_as_sf(foo) %>% 
  ...
  ggsflabel::geom_sf_label_repel(aes(label=paste(stringr::str_remove(Name, "Episcopal Church"), signif(value,2)))) +
  labs(title="SLOSH model results from NOAA, Category 1 Hurricane Storm Surge in feet") +
  coord_sf(xlim=c(box$xmin, box$xmax),c(box$ymin, box$ymax))
  coord_sf(xlim=c(-95.2, -94.75), ylim=c(29.25, 30))

ggsave("/home/ajackson/")
