...
tmap::tm_shape(Long_Convo) + 
  ...
  tmap::tm_fill(col = "Quake_freq", title = "Earthquakes", alpha=0.3, style="pretty")+
  tmap::tm_borders() 
...
Hazard_names <- tribble(~from, ~to,
                        "Lightning_freq",	 "Lightening strikes",
                        "Drought_freq",	   "Drought Days",
                        "Hail_freq",	     "Hail Storms",
                        "Wind_freq",	     "Wind Stroms",
                        "Winter_freq",	   "Winter Stroms",
...
for (i in 1:162) {
  ...
  FEMA_tract <- FEMA %>% 
    select(Tract, SocVuln, Resilience, Coastal_Flood_Freq, Landslide_freq, River_Flood_freq)
  FEMA_cty <- FEMA_county %>% 
    select(County, SocVuln_cty=SocVuln, Resil_cty=Resilience, Cold_freq, Drought_freq,
           Quake_freq, Hail_freq, Heat_freq, Hurricane_freq, Ice_freq, Lightning_freq,
  ...
  # table <-  
    ...
    gt() %>% 
    tab_header(
      title = md(paste0("**FEMA Derived Hazards for ", Label, ", ", City, "**")),
      subtitle="Annual frequency (or days per year) from Census Tract or County"
    ) %>% 
    tab_source_note(
